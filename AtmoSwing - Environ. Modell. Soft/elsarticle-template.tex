...
The Downscaler module is the last addition to AtmoSwing. Its purpose is to downscale either climate model outputs for climate impact studies or reanalyses to proceed to climate reconstruction of the past. 

The Downscaler is able to read outputs of general circulation models (GCMs) or regional climate models (RCMs), such as the Coupled Model Intercomparison Project Phase 5 \citep[CMIP5,][]{Taylor2012} and EURO-CORDEX \citep{Jacob2014}. These datasets are distributed in NetCDF format, but present a great variety of time step, temporal reference, and files structure. A complete redesign of the management of predictor data was necessary to provide the flexibility to account for varied file structures. The Downscaler is thus able to parse these datasets' original files by exploiting the self-descriptive capacity of NetCDF files.

The use of AMs in the context of future climate is rather seldom up to now due to uncertainty in the preservation of the relationship between predictors and predictands. All AMs cannot be used for this purpose, as some predictors might not capture the climate change signal well. However, \citet{Dayon2015} demonstrated the transferability of some AMs for a future climate \citep[see also][]{Dayon2018}. The transferability of an AM must be assessed before it is used in such context. 

AMs have also been used to proceed climate reconstruction of the past \citep{Caillouet2016, Caillouet2017, Bonnet2017}. Such applications allow for example hydrological modelling of flood events in periods where no meteorological data was available, or analysis of past severe droughts.
...
\begin{figure*}[t]
	\includegraphics[width=13cm]{figures/selection_variables_optim.pdf}
