...
\begin{document}
	...
	When looking for analogues in a reanalysis to target situations described by NWP or climate model outputs, certain precautions must be taken to account for different model climates and biases. The circulation variables are generally not a big issue as they are more robust and as the S1 criteria consists in a comparison of the gradients, so biases in the geopotential field values are not a big concern. Climate models with biases in the frequency of circulation types should however be avoided. Other variables that are more dependent on the model, such as humidity, might differ from one product to another. It would be better then to use the reanalysis built with the most similar model, and to work with anomalies to the model's climate rather than absolute values. Finally, instead of choosing a single reanalysis, it can be better to use several reanalyses, eventually as an ensemble. The most recent products of different institutions should be considered by default for this kind of approach.
