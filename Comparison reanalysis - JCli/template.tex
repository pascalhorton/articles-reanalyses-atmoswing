...
\abstract{
	Statistical downscaling based on a perfect prognosis approach often rely on reanalyses to infer the statistical relationship between synoptic predictors and the local variable of interest, here daily precipitation. Nowadays, many reanalyses are available and their impact on the downscaled variable is not often considered. The present work assessed the impact of ten reanalyses on the performance of seven analogue methods for statistical precipitation downscaling at 300 stations in Switzerland. Despite being in a data-rich region, significant differences were found between reanalyses and their impact on the performance of the method can even be higher than the choice of the predictor variables. There were no single overall winner, but a selection of reanalyses resulting in higher skills. 
...
%TODO: Update figs 2, 3, 4, 5, 6, 8, 10, 11, 12 with last version
...
Finally, MERRA-2 showed a striking performance with SLP, that was as skilful as using 4 geopotential heights. 



The optimal spatial domains associated with MERRA-2's SLP are inland and cover part of France, Italy, Switzerland, and a portion of the Northern Mediterranean (no domain starting before 1.25\degree W and zonal extents mainly between 5 and 10\degree; domains down to about 41\degree N with a mode at 42\degree N and a meridional extent mainly between 5 and 7\degree).
