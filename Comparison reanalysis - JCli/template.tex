...
\abstract{
	Statistical downscaling based on a perfect prognosis approach often rely on reanalyses to infer the statistical relationship between synoptic predictors and the local variable of interest, here daily precipitation. Nowadays, many reanalyses are available and their impact on the downscaled variable is not often considered. The present work assesses the impact of ten reanalysis datasets on the performance of seven analogue methods for statistical precipitation downscaling at 300 stations in Switzerland. Despite being in a data-rich region, significant differences were found between reanalyses and their impact on the performance of the method can even be higher than the choice of the predictor variables. There is no single overall winner, but a selection of datasets resulting in higher skills. 
	The methods based on geopotential heights are not highly sensitive to the spatial resolution of the output products. As expected, more local variables, such as moisture, are more sensitive to the spatial resolution. 
	Reanalyses with longer archives allow increasing the pool of potential analogues resulting in higher performances. However, when adding variables affected by errors in a more distant past, the skill decreases again.
