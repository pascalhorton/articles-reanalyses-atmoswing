...
\todo{Not transposable on precip from the reanalysis}
\todo{indirect effect -> hard to identify the reason of the impact}
\todo{check ensemble spreads vs nb analogues}
\todo{check again interannual variability}
\todo{interannual variability: RMSE ?}
\todo{do a plot of temporal annual totals}
\todo{try monthly / seasonal variability}
\todo{MERRA-2 coupled ?}
\todo{test combining results...}
\todo{perspective: using different reanalyses as members}
...
New reanalysis products tend to have higher spatial resolutions, such as the coming ERA-5, which allows better representing some processes such as convection or orographic influences. However, as shown in Sect. \ref{sec:analyzes}\ref{sec:resolution}, higher output resolutions in AMs are not a guarantee for better skills. High resolution data did not improve substantially the analogy. Nevertheless, with the increase in model resolutions, it can be expected that reanalyses will gain in accurate information at high resolutions. This might increase the skill of AMs, up to a certain level. 
